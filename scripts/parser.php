...
function parseLog($rawLog, $rawPlayerInfo) {
  $playerInfo = extractPlayerInfo(preg_split('/\r?\n/', $rawPlayerInfo));
  $log = parseHTML($rawLog);
  $log = separateLogByAction($log);
  $log = checkLogForDuplicateEndOfTurnErrors($log);
  $log = cleanLog($log);
  $log = groupActionsByPlayer($log);
  $log = determineGameTime($log, $playerInfo);
  $log = parseActions($log);
  $battleLog = groupBattleActions($log);
  $log = removeOldBattleActions($log, $battleLog);
...
function separateLogByAction($log) {
  $newLog = array();
  ...
  foreach($log as $action) {
    ...
    if(!empty(trim($action))) {
      ...
      if(preg_match('/(#[A-Z0-9]+)/', $action, $matches) === 1) {
        $action = str_replace($matches[1], matchHexCodeToColor($matches[1]), $action);
      }
...
function matchHexCodeToColor($hexCode) {
  $color = "";
  switch($hexCode) {
    case "#BFBF00":
      $color = "Yellow";
 ...
 */
function checkLogForDuplicateEndOfTurnErrors($log) {
...
function groupActionsByPlayer($log) {
  $newLog = array();
  $turnStartPattern = '/^BEGINNING OF TURN for ([a-zA-Z]+)/';
  $turnCount = -1;
...
function determineGameTime($log, $playerInfo) {
  $turnCount = 0;
  $roundCount = 0;
  $turnOrder = array();
 ...
 */
function parseActionStats($actionString, $pattern, $actionName, $data) {
  $matches = array();
  $result = array();
...
function groupBattleActions($log) {
  $battleLog = array();
...
function removeOldBattleActions($log, $battleLog) {
